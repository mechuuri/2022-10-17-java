 ...
 * 			}
 * 			
 * 			예) Movie[] movie=new Movie[1938]; ==> 데이터 많은 경우 초기화 해야됨
			= 생성자
			  class A ==> 변수의 초기화 메소드
			  {
 ...
 * 		기본값 ==> 명시적 초기화 => 클래스 블록 => 생성자
 * 	2. 인스턴스
 * 		기본값 ==> 명시적 초기화 => 인스턴스 블록 => 생성자 
 * 		
 * 		***
 * 
 * 	클래스의 구성요소
 ...
 * 			  객체 변수(인스턴스변수) ==> new 할 때마다 따로 생성하는 변수
 * 		메소드 : 저장된 데이터를 활용하는 기능
 * 		생성자 : 데이터에 대한 초기화
 * 		----------------------- 생략이 가능
 * 								생성자가 없는 경우에 데이터를 초기화 {}, static{}
 * 		==> 객체지향 (재사용, 보안) ==> 7장 (기술면접 단골)
 * 				   ---- 수정, 추가
 * 			재사용
 * 			 = 변경해서 사용 = 상속 (is-a)
 * 			 = 있는 그대로 사용 = 포함 (has-a)
