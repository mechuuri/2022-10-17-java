/*
 	...
 	--------------------
 		...
 		*** DB 연동시 가장 많이 사용되는 컬렉션
 		*** 주요 기능
 		 	add()= 추가, get()= 읽기, size()= 저장 갯수, 
 		 	isEmpty()= 데이터존재 여부, clear()= 전체 삭제
 		*** 크롤링
 		
 		Vector : 동기화 프로그램 (네트워크에서 주로 사용됨)
 				 Vector의 단점을 보완 => ArrayList이다 (주로 사용)
 		LinkedList : 연결기반 (메모리 크기가 크다 (다음에 있는 데이터 주소, 본인 데이터 주소)
 					 데이터 추가, 삭제시 속도가 빠름, 데이터에 대한 접근성이 낮음 
 		*** 메소드가 동일
 		...
 		Properties : 파일로 만들어진다
 			주요기능) load() : 파일 읽기, getProperty() : 파일에서 데이터 읽기
