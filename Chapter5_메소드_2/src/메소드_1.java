 ...
 * 	2. 사용하는 이유
 * 		=> 재사용
 * 		=> 가독성 (소스를 관련된 내용으로 나눈다) => 문장의 단락을 나눠서 처리하는 것과 같다! (구조적 프로그램)
 * 		=> 중복코드 제거
 * 	3. 메소드 구조
 ...
 * 			------ 생략할 수 있다 (리턴형이 void일 때) void => 자체에서 겨로가값 출력
 * 		}
 * 		
 * 		*** return은 메소드 종료를 알려준다
 * 			return은 항상 마지막에 추가되는 것은 아니다. ==> 조건에 맞는 return 설정 가능
 * 	
 * 	4. 메소드 유형
 * 	-----------------
 * 	리턴형 	매개변수(지역변수의 일종) ===> 메소드종료와 동시에 메모리에서 사라지는 변수
 * 			=> 메소드 안에서만 사용이 가능
 * 	-----------------
 * 	  O		  O 
 * 		라이브러리
...
static void sort(int[] arr)
	...
	static void gugudan()
	{
		for(int i=1;i<=9;i++)
		{
			...
			System.out.println(); //받을 건지 프로그램 요청할 건지
		}
	}
