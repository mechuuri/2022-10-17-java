 ...
 *				형식) if(조건문)
 *					{
 *						조건이 true일 때 수행하는 문장; (false면 건너 뜀)
 *					}
 *
 ...
 *						location.href="/main"; 
 *		= 반복문
 *			=> for ====> 반복 횟수를 알고 있는 경우 => 이중 for문 (중첩 for문)
 *			     형식)
 *						  1 →  2   ← 4
 *					for(초기값;범위;증감식)
 *							  ↓ 3
 ...
 *
 *			=> while ===> 반복 횟수를 모르는 경우 => 무한루프 (네트워크 서버, 데이터베이스 값 가지고 올 때) => 서버단 
 *				형식) 
 *					초기값 			====> 1
 *					while(조건식)		====> 2	false 종료 ==> 수행이 안될 수 있다
 *					{
 *							| true
 *						반복 수행문장	====> 3
 *						증감식		====> 4	======= 증가된 값 =======> 조건식
 ...
 *						{
 *						}
 *			=> do~while ==> 후 조건(조건 나중에 검색) > 반드시 한번 이상 수행이 가능 ==> 웹에서 거의 사용빈도가 없다
 *					형식) 
 *						초기값		=====> 1
 *						do
 *						{
