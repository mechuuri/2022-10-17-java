 ...
 * 			클래스 메모리 저장
 * 				클래스명 변수명=new 클래스명()
 * 				----------------------- new 연산자는 새로운 메모리 만들어줌
 * 
 * 			1) 변수
 * 			***	인스턴스 변수 : new를 이용할 때마다 따로 생성되는 변수 (값 저장 시 다른 데이터값을 가지고 있다)
 * 				클래스 변수 : 컴파일러에 의해 자동 생성되는 변수 => 메모리 공간이 한개이기 때문에 모든 데이터값이 동일
 * 							공유할 때 사용
 * 							static 사용 (네트워크) => 서버 (접속자 정보) => IP
 * 				==> 모든 데이터가 static : oracle
 * 				지역변수(매개변수): 메소드 안에서 선언 => 메소드 호출이 종료하면 자동으로 사라지는 변수
 * 				---------------------------	for, while
 * 
